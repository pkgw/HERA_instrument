...
(see \cite{parsons_et_al2012b}).  Fig. \ref{fig:boost} plots this ``boost'' factor for various telescopes under nominal observing campaigns using the delay-spectrum technique.  The large improvement for PAPER and HERA is the fact that the baselines are redundantly placed on appropriate spatial scales, so the same baselines gets many samples.
...
\caption{\small ADD NEW HERA CONFIGURATION AND LOFAR AND PUT IN 'HOW TO MAKE THIS PLOT'}
\label{fig:kperf}
\end{figure}
