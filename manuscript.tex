...
HERA brings to bear both the sensitivity and the precision to deliver its key science results on the basis of proven techniques.
...
staged deployment.  The key is to produce inexpensive collecting area optimized to detect the EOR.  This has resulted in a close-packed array of transit 14-meter dishes (Fig. \ref{fig:HERApictures}), with the first 19-element array completed and construction underway for 37 elements with additional funding from Cambridge University. 

The current 19 elements are testing the HERA element design for performance and manufacturability at location.  The 37 elements, to be completed in 2016, will provide nearly an order of magnitude more 
...
This paper has the following structure.  Section \ref{sec:science} will give a high-level overview of the science motivating HERA's construction, including primary and secondary science objectives.  Section \ref{sec:eormeas} will present the techniques used to make the measurement, focussing on the delay-spectrum approach.  Section \ref{sec:existing} will provide a brief summary of the existing measured limits. Section \ref{sec:requirements} will present the high-level requirements and Section \ref{sec:design} will present the system description..   Section \ref{sec:status} will present a brief status and outline the deployement timetable and Section \ref{sec:conclusion} will conclude by summarizing and providing additional context.

\section{Scientific Background}
\label{sec:science}

\noindent The {\it Cosmic Dawn} of our universe is one of the last unexplored
frontiers in cosmic history, which is summarized in Fig. \ref{fig:cosmos}. On the left is the Big Bang, then the hot, young universe slowly cools until it begins to gravitationally contract to the first structures that relit the universe.
The Cosmic Dawn saw the evolution of the universe from a very homogeneous ``gas'' to the formation of the first stars and galaxies en route to the structure of galaxies
of our current time.
...
the MIT EoR experiment (MITEoR; \citealt{zheng_et_al2014}) and the Experiment to Detect the Global EoR Step (EDGES; \citealt{bowman_rogers2010}) (see Sec. \ref{sec:existing}).
Recent measurements
...
therefore equates to a cosmic distance along the site of the telescope, with the frequency determining the cosmic age.  An observation of an area of sky over a given bandwidth is therefore providing an average
...
Though the power spectrum measurement is over a volume, it is instructive to split the spatial wavenumbers into two components,  $\kvec =  \kvpr + \kpar{\hat {\bf z}}$ where $\kvpr$ is determined by the antenna baseline and $\kpar$ by the bandwidth.  This is useful since it allows us to split out the chromatic response of the interferometer and isolate a phase space where foreground sources ({\em i.e.} everything {\em not} the EOR) contaminate the signal of interest from where they don't.  As mentioned above, this contaminated phase space may be shown [REF] to be a wedge-shaped region in $\kpr$:$\kpar$ space:
\begin{equation}
...
from \cite{mesinger_et_al2011}) to the upper left should be free from foregrounds.}
\label{fig:wedge}
\end{figure}
...
Fig. \ref{fig:boost} (left) plots this ``redundancy boost'' factor for various configuration types relative to an array in a 19-element hexagonal configuration.  They are labelled for the type of configuration (hex, grid, imaging) and the number of antennas (from 37 to 350).  Note that the (a) hex is based on the configuration used for HERA (14.6m center-to-center spacing); (b) grid for that of PAPER (4m N-S, 16m E-W spacing); (c) Imaging-128 for MWA (\cite{mwa}); and (d) Imaging-48 for LOFAR (\cite{2013A&A...556A...2V}; note that this assumes the ability to fully correlate the HBA-pairs).  The plot shows that for a fixed number using the delay-spectrum approach, redundant arrays (hex/grid) provide about an order of magnitude improvement over imaging arrays.  Since the baselines go as order $N^2$, this corresponds to using about 1/3 of the number of elements to yield the same performance (as verified by the Hex-37/Imaging-128 lines).  This advantage does not extend to the other approaches discussed below and imaging configurations would be preferred.  Note that a filled hexagonal-packed array provides excellent imaging as well, but with potentially limited resolution unless outriggers are includes.

Another approach to improving sensitivity is to make the collecting area per element larger.  In the limit, this yields a fully filled aperture as mentioned.  A larger element limits the accessible field-of-view, which however is not a huge liability for a focussed experiment, as long as an appropriate patch of ``cold'' pass passes within its field.  One method combining both advantages is to use small elements to build up phased array ``tiles'' and then electronically produce many beams, which may get correlated.  This adds additional complexity and potential systematics, however it is the method employed by both MWA and LOFAR.  Fig. \ref{fig:boost} (right) shows the relative sensitivity of the various array relative to HERA-19.  The circles near the array name indicate the relative sizes of the element.  Note that MWA and LOFAR are cross-hatched to indicated phased array tiles.
...
The primary new feature of HERA over previous generation experiments (in this context, primarily PAPER) is the use of large elements to increase the sensitivity.  The cost-performance optimization will 
...
should be able to accommodate ~1km baselines.  In referring to the EOR sensitivity of the array however, the number of antennas just in the core is sometimes used.
...
\label{sec:cost}
As an instrument to characterize the power spectrum over its evolution, the specification on sensitivity is to make at least a nominal detection over the redshift region of support, or $z$=6-13, along with a very robust detection at the peak.  As seen in Fig. \ref{fig:Sensitivities} and Table \ref{tab:signif}, an array comprising 320 14-m core antennas (note, labelled HERA-350, which includes the outriggers) has sensitivity to a
...
\end{equation}
where $N_{\text core}$ and $D$ are the number of antennas in the core and their diameter respectively.  Note that the number-diameter dependence was derived by running multiple sensitivity codes ({\em e.g.} Pober) over a range of values for the HERA configuration.  This dependency differs from Mellema and analytical epressions, mostly likely due to the assumed configuration.
Optimization is 
therefore done for this sensitivity against cost for various diameters and total element numbers, while trying to minimize multiple reflections on long delays.  Figure \ref{fig:costfig} summarizes this optimization, which
is discussed below.

To determine the optimal diameter, full system costings were done on a range of diameter sizes from 5m - 25m, where the total number was constrained to keep constant sensitivity at the minimum discussed in the previous paragraph.  This included the full system, except the post-processing (that is, everything after the archived correlator output).  
