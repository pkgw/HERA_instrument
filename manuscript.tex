...
providing evidence for significant heating in the IGM prior to reionization \citep{parsons_etal2014, 2015ApJ...809...61A,pober_et_al2015}.
...
Foreground avoidance represents an analysis comparable to \cite{2015ApJ...809...61A}, whereas foreground modeling allows signifcantly more $k$ modes of the cosmological signal to be recovered.}
\small
 \centering
...
\citep{dillon_et_al2015,parsons_etal2014,2015ApJ...809...61A}, with PAPER
...
appear there through instrumental leakage \citep{parsons_etal2014,2015,ApJ...809...61A,moore_et_al2016,kohn_et_al2016}.
...
of magnitude (in mK) of these models \citep{2015ApJ...809...61A} and show the ``EoR Window'' to be foreground-free.
...
(see \cite{parsons_et_al2012b}).  Fig. \ref{fig:boost} plots this ``boost'' factor for various configuration types relative to an array in a 19-element hexagonal configuration.  They are labelled for the type of configuration (hex, grid, imaging) and the number of antennas (from 37 to 350).  Note that the hex is based on the configuration used for HERA, grid for that of PAPER, Imaging-128 for MWA and Imaging-48 for LOFAR.  The plot shows that for a fixed number using the delay-spectrum approach, redundant arrays (hex/grid) provide about an order of magnitude improvement over imaging arrays.  Since the baselines go as order $N^2$, that corresponds to that for about 1/3 the number of elements you get the same performance (as verified by the Hex-37/Imaging-128 lines).  This advantage does not extend to the other approaches discussed below.

Another approach to improving sensitivity is to make the collecting area per element larger.
...
magnitude improvement in delay-spectrum sensitivity.  Alternatively, for the same sensitivity one can build roughly 1/3 the antennas.  Right:  Power spectrum sensitivities for various arrays normalized to the
19-element hex.  Here the element size is included, which is shown as an appropriately sized circle.}
\label{fig:boost}
\end{figure}
...
\section{Existing Measurements}
\label{sec:existing}
As mentioned earlier, a number of programs are underway to make the first detection of the EoR power spectrum using the above techniques.  
...
\citep{zheng_et_al2014,parsons_etal2014,2015ApJ...809...61A}.
